...
// Snake above the Claude Code prompt, split out of cc-arcade (https://github.com/sezaakgun/cc-arcade,
// MIT, by Seza Akgün) as a single-game mod. `/snake` opens it, `/snake stop` closes it. The board in
...
const GAME = 'snake' // what the board posts its score under
const TITLE = 'Snake'
...
export const register: Register = on => {
  on('session.start', async ($, e, next) => {
    ...
    await $.command.register({
      name: 'snake',
      description: `$Snake above the prompt, while Claude works (stop closes)`,
      argumentHint: '[stop]',
      immediate: true,
    }).catch(err => $.ui.log(`snake: /snake not registered: ${err}`))
    return r
  })
  ...
  on('command.run', { command: 'snake' }, async ($, e) => {
    ...
    if (arg === 'stop' || arg === 'close') {
      ...
      return { text: `$Snake closed` }
    }
    open = true
    $.ui.invalidate('ui.render')
    const score = best ? ` · best ${best}$` : ''
    return { text: `$Snake · click the board to play · Esc returns to the prompt · /snake stop closes${score}` }
  })
  ...
  on('ui.message', async ($, e, next) => {
    ...
    if (isBetter(data.score, best || undefined, LOWER_IS_BETTER)) {
      best = data.score
      await $.store.set('best', best).catch(err => $.ui.log(`snake: store write failed: ${err}`))
      $.ui.toast(`$Snake: new best ${best}$`)
    }
    ...
    return (
      <Box flexDirection="column">
        <Box flexDirection="row" columnGap={1}>
          <Button key="snake:close" label="close" onPress={close} />
        </Box>
        <Client key="board:snake" module="./boards/snake.tsx" width={cols} height={rows} props={{ best, done: turnsDone, safe: false }} />
        {await next(e)}
      </Box>
    )
  })
